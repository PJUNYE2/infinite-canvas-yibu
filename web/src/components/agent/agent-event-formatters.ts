...
export function formatAgentEventLog(event: AgentEventPayload) {
    ...
    if (event.type === "item.completed" && item && isMcpToolItem(item)) return { title: tr(item.error ? "toolFailed" : "toolCompleted"), text: `${toolName(String(item?.tool || ""))}${item.error?.message ? ` · ${item.error.message}` : ""}` };
