...
function ErrorContent({ node, theme, onRetry }: Pick<NodeContentRendererProps, "node" | "theme" | "onRetry">) {
    ...
    return (
        <div className="flex max-w-[260px] flex-col items-center gap-3 px-5 text-center">
            <div className={`text-xs leading-5 ${recoverable ? "text-green-300" : "text-red-300"}`}>{node.metadata?.errorDetails || t("canvas.node.failed")}</div>
            ...
            >
                ...
                {t(recoverable ? "canvas.node.resume" : "canvas.node.retry")}
