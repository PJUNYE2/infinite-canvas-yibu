...
import { createModelChannel, guessCapability, modelOptionsFromChannels, normalizeModelOptionValue, selectableModelsByCapability, useConfigStore, type AiConfig, type ApiCallFormat, type ConfigTabKey, type ModelCapability, type ModelChannel } from "@/stores/use-config-store";
...
export function AppConfigPanel({ showDoneButton = false, initialTab = "channels" }: { showDoneButton?: boolean; initialTab?: ConfigTabKey }) {
    ...
    const autoDetectModels = () => {
        const channels = config.channels.map((channel) => ({ ...channel, models: channel.models.map((model) => ({ ...model, capability: guessCapability(model.name) })) }));
        const nextConfig = { ...config, channels };
        const counts: Record<ModelCapability, number> = { image: 0, video: 0, text: 0, audio: 0 };
        channels.forEach((channel) => channel.models.forEach((model) => counts[model.capability]++));
        const defaults = modelGroups.reduce<Partial<AiConfig>>((values, group) => {
            const options = selectableModelsByCapability(nextConfig, group.capability);
            const current = normalizeModelOptionValue(config[group.modelKey], channels);
            values[group.modelKey] = current && options.includes(current) ? current : options[0] || "";
            return values;
        }, {});
        saveConfig({ ...nextConfig, ...defaults });
        message.success(`已自动识别：生图 ${counts.image}，视频 ${counts.video}，文本 ${counts.text}，音频 ${counts.audio}`);
    };
                    ...
                    },
                    {
                        ...
                        children: (
                            <Form layout="vertical" requiredMark={false}>
                                ...
                                <div className="mb-3 flex flex-wrap items-center justify-between gap-3"><div><div className="text-sm font-semibold">{t("config.preferences.defaultModels")}</div><div className="mt-1 text-xs text-stone-500">{t("config.preferences.autoDetectDescription")}</div></div><Button icon={<RefreshCw className="size-4" />} onClick={autoDetectModels}>{t("config.preferences.autoDetect")}</Button></div>
