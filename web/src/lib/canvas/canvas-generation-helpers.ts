...
export function resetInterruptedGeneration(nodes: CanvasNodeData[]) {
    return nodes.map((node) => {
        ...
        return { ...node, metadata: { ...node.metadata, status: "error" as const, errorDetails: taskId ? i18n.t("canvas.generation.taskRecoverable", { taskId }) : i18n.t("canvas.generation.interrupted"), asyncTaskRecoverable: Boolean(taskId) } };
    });
}
