...
function InfiniteCanvasPage() {
    ...
    const imageRequestOptions = useCallback((nodeId: string, signal: AbortSignal) => ({
        signal,
        onTaskCreated: (taskId: string) => setNodes((prev) => prev.map((node) => node.id === nodeId ? { ...node, metadata: { ...node.metadata, asyncTaskId: taskId, asyncTaskRecoverable: true } } : node)),
    }), []);
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            try {
                const image = await requestEdit(generationConfig, prompt, [source], { id: `${node.id}-mask`, name: "mask.png", type: "image/png", dataUrl: payload.maskDataUrl }, imageRequestOptions(childId, controller.signal)).then((items) => items[0]);
        ...
        async (node: CanvasNodeData, params: CanvasImageAngleParams) => {
            ...
            try {
                const image = await requestEdit(
                    ...
                    [{ id: node.id, name: `${node.title || node.id}.png`, type: node.metadata.mimeType || "image/png", dataUrl: node.metadata.content, storageKey: node.metadata.storageKey }],
                    undefined,
                    imageRequestOptions(childId, controller.signal),
                ).then((items) => items[0]);
                const uploaded = await uploadImage(image.dataUrl);
                ...
                const errorDetails = error instanceof Error ? (taskId ? `${error.message}。任务ID:${taskId}` : error.message) : t("canvas.projectPage.generationFailed");
                setNodes((prev) => prev.map((item) => (item.id === childId ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_ERROR, errorDetails, asyncTaskId: taskId, asyncTaskRecoverable: Boolean(taskId) } } : item)));
        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            if (sourceNode && builtinPanel?.writeBackToSelf && builtinPanel.mode === "image") {
                ...
                try {
                    ...
                    const image = refs.length
                        ? await requestEdit({ ...generationConfig, count: "1" }, fullPrompt, refs, undefined, imageRequestOptions(nodeId, controller.signal)).then((items) => items[0])
                        : await requestGeneration({ ...generationConfig, count: "1" }, fullPrompt, imageRequestOptions(nodeId, controller.signal)).then((items) => items[0]);
                    const uploaded = await uploadImage(image.dataUrl);
                    setNodes((prev) =>
            ...
            try {
                if (mode === "image") {
                    ...
                    let firstError = "";
                    let recoverableTaskId = "";
                    await Promise.all(
                        targetIds.map(async (targetId) => {
                            try {
                                const image = referenceImages.length
                                    ? await requestEdit({ ...generationConfig, count: "1" }, effectivePrompt, referenceImages, undefined, imageRequestOptions(targetId, controller.signal)).then((items) => items[0])
                                    : await requestGeneration({ ...generationConfig, count: "1" }, effectivePrompt, imageRequestOptions(targetId, controller.signal)).then((items) => items[0]);
                                const uploaded = await uploadImage(image.dataUrl);
                    ...
                    setNodes((prev) =>
                        prev.map((node) =>
                            node.id === nodeId && isConfigNode
                                ...
                                ? { ...node, metadata: { ...node.metadata, status: hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR, errorDetails: hasSuccess ? undefined : recoverableTaskId ? t("canvas.generation.taskRecoverable", { taskId: recoverableTaskId }) : t("canvas.projectPage.generationFailed"), asyncTaskId: hasSuccess ? undefined : recoverableTaskId || undefined, asyncTaskRecoverable: !hasSuccess && Boolean(recoverableTaskId) } }
                                : node.id === nodeId && isEmptyImageNode
                                  ? { ...node, metadata: { ...node.metadata, status: hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR, errorDetails: hasSuccess ? undefined : recoverableTaskId ? t("canvas.generation.taskRecoverable", { taskId: recoverableTaskId }) : t("canvas.projectPage.generationFailed"), asyncTaskId: hasSuccess ? undefined : recoverableTaskId || undefined, asyncTaskRecoverable: !hasSuccess && Boolean(recoverableTaskId) } }
                                  : node.id === rootId && !hasSuccess && !targetIds.includes(node.id)
                                    ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, errorDetails: recoverableTaskId ? t("canvas.generation.taskRecoverable", { taskId: recoverableTaskId }) : t("canvas.projectPage.allFailed"), asyncTaskId: recoverableTaskId || undefined, asyncTaskRecoverable: Boolean(recoverableTaskId) } }
        ...
        async (node: CanvasNodeData) => {
            ...
            if (node.type === CanvasNodeType.Image && node.metadata?.asyncTaskId) {
                ...
                try {
                    ...
                    const imageConfig = NODE_DEFAULT_SIZE[CanvasNodeType.Image];
                    const size = fitNodeSize(uploaded.width, uploaded.height, imageConfig.width, imageConfig.height);
                    setNodes((prev) => prev.map((item) => item.id === node.id ? { ...item, width: size.width, height: size.height, metadata: { ...item.metadata, ...imageMetadata(uploaded) } } : item));
                } catch (error) {
                    if (isGenerationCanceled(error)) return;
            ...
            try {
                ...
                const image = useReferenceImages
                    ? await requestEdit(generationConfig, prompt, retryImages, undefined, imageRequestOptions(node.id, controller.signal)).then((items) => items[0])
                    : await requestGeneration(generationConfig, prompt, imageRequestOptions(node.id, controller.signal)).then((items) => items[0]);
