    ...
    const runGenerationSlot = async (index: number, snapshot: { text: string; config: AiConfig; references: ReferenceImage[] }) => {
        ...
        try {
            ...
            if (isImageTaskPollingError(error)) {
                setResults((value) => updateResultAt(value, index, { status: "failed", taskId: error.taskId, recoverable: true, error: t("imageWorkbench.taskRecoverable", { message: error.message, taskId: error.taskId }) }));
            } else {
                setResults((value) => updateResultAt(value, index, { status: "failed", error: error instanceof Error ? error.message : t("workbench.generationFailed"), recoverable: false }));
            }
    ...
    const resumeResult = (index: number, taskId: string) => {
        ...
        void resumeImageTask(snapshot.config, taskId)
            ...
            .catch((error) => {
                ...
                setResults((value) => updateResultAt(value, index, { status: "failed", taskId: recoverable ? taskId : undefined, recoverable, error: recoverable ? t("imageWorkbench.taskRecoverableShort", { taskId }) : error instanceof Error ? error.message : "查询失败" }));
            });
    };
                            ...
                            <div className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-3">
                                {results.map((result, index) =>
                                    result.status === "success" && result.image ? (
                                        ...
                                        <FailedImageCard key={result.id} error={result.error || t("workbench.generationFailed")} taskId={result.taskId} recoverable={result.recoverable} onResume={result.taskId ? () => resumeResult(index, result.taskId!) : undefined} onRetry={() => retryResult(index)} />
    ...
    return (
        <div className={`overflow-hidden rounded-lg border ${recoverable ? "border-green-200 bg-green-50 dark:border-green-950 dark:bg-green-950/20" : "border-red-200 bg-red-50 dark:border-red-950 dark:bg-red-950/20"}`}>
            <div className="flex aspect-square flex-col items-center justify-center gap-3 p-5 text-center">
                ...
                <div className={`text-sm font-medium ${recoverable ? "text-green-700 dark:text-green-300" : "text-red-600 dark:text-red-300"}`}>{recoverable ? t("imageWorkbench.taskSubmitted") : t("workbench.failed")}</div>
            ...
            <div className={`flex justify-end gap-2 border-t p-3 ${recoverable ? "border-green-200 dark:border-green-950" : "border-red-200 dark:border-red-950"}`}>
                {recoverable && onResume ? (
                    <Button size="small" type="primary" onClick={onResume}>
                        {t("imageWorkbench.continueQuery")}
                    </Button>
                ) : null}
                <Button size="small" danger={!recoverable} onClick={onRetry}>
                    {t("imageWorkbench.regenerate")}
